...
> **الحالة:** Phase 1 (P-01) **شغّال end-to-end** ✅ — 5 خدمات محاواة + ذاكرة per-user (L1) + رؤية (صور) + اختبارات/CI. الواجهة على http://127.0.0.1:3000.
>
...
| [docs/DECISIONS.md](docs/DECISIONS.md) | سجل القرارات المعمارية (ADR-001..018) |
...
## ▶️ التشغيل
من **جذر المستودع** (شرط مسبق: Docker Desktop بخلفية WSL2 + تعريف NVIDIA يدعم GPU):
...
البنية: المشروع في جذر المستودع؛ `legacy/` = النموذج الأولي المؤرشَف (Qwen3/Gemma، غير مُشغَّل، خارج البوّابة).

**التالي:** تقوية P-01 (virtual key بدل master-key، تثبيت digests الصور) · ذاكرة Phase 2 (استرجاع دلالي). [✓ الرؤية مفعّلة [ADR-014](docs/DECISIONS.md) · ✓ اختبارات + CI]. كل التفاصيل وحالة البنود في [docs/PROGRESS_MAP.md](docs/PROGRESS_MAP.md).
