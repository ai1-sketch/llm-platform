...
- **R-ARCH-10** — **العميل لا يعرف المحرّك.** أي frontend / app / agent / script يتّصل **حصراً** بعنوان البوّابة (`http://litellm:4000/v1`). فحص: `api_base`/`base_url` في أي عميل = منفذ البوّابة فقط، لا منفذ المحرّك (`vllm:8000`). **يشمل ذلك التضمين** (القاعدة تبقى سارية لأي عميل/تكامل). مثال محرّك السياق (`memory`→`embed-default` عبر البوّابة، [ADR-023](DECISIONS.md)) متقاعد إلى فرع `future/context-engine` ([ADR-025](DECISIONS.md)). **ملاحظة صدق (تدقيق 2026-07-10):** تضمين OWUI الحالي (all-MiniLM) يجري **محلياً داخل OWUI خارج البوّابة** ([ADR-025](DECISIONS.md)) — استثناء حيّ قائم لهذه القاعدة، قيد المراجعة كبذرة "توجيه التضمين عبر البوّابة" (لم يُحسم بعد؛ يتطلّب ADR).
...
- **R-ARCH-31** — **أسماء خدمات Docker حياديّة المزوّد وصريحة الدور:** **(5 أساسية مطلوبة)** `open-webui`, `litellm`, `vllm`, `postgres-litellm`, `postgres-openwebui` **+ (2 مراقبة اختيارية قابلة للحذف)** `prometheus`, `grafana` (`profiles: [monitoring]` — [ADR-031](DECISIONS.md)/[ADR-033](DECISIONS.md)؛ المحرّك `vllm` منذ [ADR-028](DECISIONS.md)؛ قاعدتا postgres مخصّصتان منذ [ADR-030](DECISIONS.md)؛ `memory`/`embeddings` متقاعدتان إلى فرع `future/context-engine` بعد [ADR-025](DECISIONS.md)). ممنوع أسماء غامضة (`app`, `server`, `svc1`) أو أسماء دور عامة (`gateway`, `engine`, `webui`, `db`). فحص: مفاتيح `services:` ⊆ القائمة (7)؛ الأساسية الخمس **مطلوبة** والمراقبة **غير مطلوبة** (حذفها لا يكسر CI — CORE/OPTIONAL في `check_invariants.py`، ADR-033). **مُحلّ:** مقاطع PROJECT_BLUEPRINT §10.2 تستخدم أسماء توضيحية (`engine/gateway/webui/db`)، وقد وُسِمت صراحةً كأمثلة إيضاحية مع الإشارة للأسماء المعتمدة ومصدر الحقيقة (`compose/docker-compose.yml`) — فلا تناقض فعليّاً مع القاعدة.
