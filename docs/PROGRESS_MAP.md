...
| **P-01** | `compose/docker-compose.yml` يشغّل `open-webui` + `litellm` + المحرّك المحلي (`vllm` منذ [ADR-028](DECISIONS.md)؛ كان `llamacpp`) + `postgres` بـ `docker compose up`. | [x] **شغّال end-to-end** (4 خدمات healthy، GPU داخل الحاوية، طلب عبر البوّابة نجح، الواجهة على :3000؛ أُعيد التحقّق حيّاً بعد ADR-028) |
...
| **التخزين الأساسي: PG+pgvector** | تخزين OWUI على PostgreSQL+pgvector (بدل SQLite/Chroma)؛ محلي الآن، قابل للانتقال لخارجي/مُدار | **ADR-029** |
