...
> **آخر تحديث:** 2026-07-10 · **المحدِّث:** Chief Architect (الموجتان A+B من التدقيق الخصمي دُمجتا على master — A: تقسية شبكة/امتياز + مواءمة حوكمة [ADR-033](DECISIONS.md) + grafana من EOL؛ B: حدود مفاتيح + نسخ مشفّرة + RTBF + watchdog آمن؛ المتانة المؤسّسية مؤجَّلة P-12 → K8s)
...
- [x] **🧱 الموجة B من التدقيق الخصمي — حدود مفاتيح + متانة النسخ + تعافٍ آمن (2026-07-10، دُمجت على master):** **B1** حدود على مفتاح OWUI (`rpm_limit=240`/`max_parallel=16`/`max_budget=100`/`30d` — مُطبَّقة حيّاً + قالب توليد المفتاح في RUNBOOK): حماية المحرّك المشترك من الإغراق + سقف كلفة جاهز لأي مزوّد مدفوع. **B2** نسخ `pg_dump` مشفّرة **AES-256** (openssl، `BACKUP_PASSPHRASE`) + تدوير 30 يوماً + جدول **RTBF** (أين تعيش بيانات المستخدم عبر 4 مخازن + استعلام حذف SpendLogs). **B3** تعافٍ ذاتي آمن = **watchdog على المضيف** (يقرأ حكم صحّة Docker، بلا docker socket = لا سطح امتياز؛ رُفض autoheal لخطره) موثّق في RUNBOOK وغير مفعّل (التسجيل قرار استمرارية للمالك)؛ المتانة المؤسّسية (HA/K8s liveness probes) مؤجَّلة **P-12** بمحفّز (المالك رفض التحويل لـ K8s الآن — §5). **مُتحقَّق حيّاً:** حدود المفتاح + دردشة 200، دورة نسخ→تشفير→فكّ (الكلمة الخاطئة لا تسرّب)، watchdog يعيد حاوية unhealthy دون لمس الصحّية. §5: تقوية لأشياء قائمة، بلا ADR جديد (كإصلاحات Codex).
- [x] **🛡️ الموجة A من التدقيق الخصمي — تقسية + مواءمة حوكمة (2026-07-10، [ADR-033](DECISIONS.md)، دُمجت على master):** تدقيق خصمي (14 عدسة + دحض + 5 محقّقين) أنتج موجة نظافة مُتحقَّقة حيّاً — **(A2) عزل الشبكة:** `internal:true` للقاعدتين+البوّابة+المراقبة و`egress` فقط لـ vllm/open-webui (سحب الأوزان/التضمين)؛ **مُثبَت حيّاً:** قاعدتا الـPII→إنترنت **BLOCKED**، open-webui **REACHED**. **(A3)** `no-new-privileges:true` على الخدمات السبع. **(A4) مواءمة نصّ↔منفّذ:** فصل **CORE(5 مطلوبة)/OPTIONAL(2 مراقبة)** في `check_invariants` — حذف المراقبة لم يعد يكسر CI (**يعيد قابلية عكس ADR-031 التي قُبِل عليها §5**؛ اختبار سلبي أخضر) + مزامنة R-ARCH-31/رأس DECISIONS (001..033)/README/تعليقات. **(A5)** ترقية grafana 11.4.1 (EOL) → **12.3.8** (digest حقيقي) + air-gap للتطبيقات. **(A1)** تقسية ACL لـ`.env` محلياً. **مُتحقَّق حيّاً:** 7 خدمات healthy، litellm→vllm 200، prometheus target up، chat e2e 200، grafana datasource+لوحة "vLLM Inference" محمّلة. ديون نصّية متبقّية (BLUEPRINT/llama.cpp، R-ARCH-24/litellm، اختبار check_invariants) موثّقة في ADR-033 لطوبة توثيق لاحقة.
- [x] **📊 طبقة رصد الأداء (النوع الثاني) + تصحيح رصد الأخطاء (2026-07-09، [ADR-031](DECISIONS.md)/[ADR-032](DECISIONS.md)، قرار المالك — دُمج على master):** بُني النوع الثاني: **Prometheus + Grafana** خلف `profiles: [monitoring]` (لا يقلعان افتراضياً؛ قابلان للحذف بعلَم) يكشط vLLM `/metrics` (KV%/طابور/TTFT/tokens-s/preemptions/prefix-cache) + لوحة "vLLM Inference" ككود. **مُتحقَّق حيّاً:** prometheus يكشط vLLM + litellm الحيّين (targets up)، استعلام Grafana→Prometheus ناجح، litellm `/metrics` مُفعّل عبر callback (بمفتاح token)، prefix caching مُفعّل (94% إصابة تحت حمل)، ونسب الإنفاق per-user مُثبَتة (dept-ali/ahmad مفصولان). والنوع الأول صُحّح: تضييق ادّعاء R-ERR-15/16/19 (الانتشار عبر الطبقات مؤجَّل — تقاعد memory/ADR-025) + ضبط خصوصية stdout على INFO (حُذف `turn_off_message_logging` كـ no-op مخالف §5) + دوران سجلّات + إنفاق per-user + BLE001. الحارس/CI/‏.env.example محدَّثة (7 خدمات). كل بوّابات CI + التحقّق الحيّ خضراء.
...
| **P-11** | **مراقبة سلوك الموديل/محتوى الطلبات** (تتبّع + فحص برومبت/رد + eval): مؤجَّلة كميزة مستقبلية — تخالف §5 + تخلق ثغرة RBAC (مخزن محتوى يتجاوز عزل الأقسام)؛ الميتاداتا مغطّاة أصلاً. محفّزات + شروط في [ADR-032](DECISIONS.md). | [ ] مؤجَّل بمحفّز |
| **P-13** | **توجيه تضمين OWUI عبر البوّابة + حسم موديل التضمين الإنتاجي:** حاليّاً OWUI يضمّن محليّاً على CPU (all-MiniLM-384 — استثناء موثّق [ADR-025](DECISIONS.md)، يخالف R-ARCH-10 لكن **يحرّر الـ6GB للموديل الرئيسي**) → **يُترَك كما هو**. التوجيه عبر البوّابة الآن يتطلّب خدمة تضمين (لا تتّسع على 6GB / خدمة جديدة = §5)، ومكسبه (قابلية التبديل) موجود أصلاً في config OWUI، وكلفة إعادة التضمين **فيزياء لا مفرّ منها** بأي حال. **البذرة الحقيقية:** حسم موديل التضمين الإنتاجي **قبل تضمين بيانات ضخمة**. **المحفّز:** أوّل قسم حقيقي (يقترن بالموجة D) / GPU أكبر / قرار تحسين جودة التضمين. **قابل للتنفيذ الآن (منفصل — جودة لا معمار):** فحص جودة الاسترجاع العربي؛ إن ضعُف = تبديل موديل تضمين محلّي أفضل داخل OWUI (config فقط). رُصد في تدقيق 2026-07 (الموجة C). | [ ] مؤجَّل بمحفّز |
| **P-12** | **المتانة/التوفّر المؤسّسي (تعافٍ تلقائي + HA + توسّع أفقي):** لا إعادة تشغيل تلقائية عند تعليق حاوية حيّة (`unhealthy`)، ولا failover متعدّد العقد، ولا نسخ خلف LB — مؤجَّلة ([ADR-007](DECISIONS.md)). **الحلّ الإنتاجي:** K8s **liveness/readiness/startup probes + N نسخ + rolling/PDB** (باب عكوس — البنية بلا حالة + عقد `/v1` تجعله هجرة تنسيق لا إعادة بناء؛ لا نبنيه الآن = §5). **جسر مؤقّت للطيّار:** watchdog على المضيف موثّق في [RUNBOOK](RUNBOOK.md) (بلا docker socket؛ **غير مفعّل** — قرار استمرارية للمالك). **المحفّز:** أوّل نشر إنتاجي حقيقي / حاجة تعدّد عقد أو توسّع أفقي (رُصد في تدقيق 2026-07). | [ ] مؤجَّل بمحفّز |
...
| **مراقبة المحتوى: مؤجَّلة** | التقاط محتوى الطلبات/التقييم = ميزة مستقبلية (§5 + أمان: ثغرة RBAC)؛ محفّزات + شروط | **ADR-032** |
| **مواءمة الحوكمة + تقسية (الموجة A)** | فصل CORE/OPTIONAL (يعيد قابلية عكس ADR-031) + عزل شبكة + no-new-privileges + grafana من EOL | **ADR-033** |
