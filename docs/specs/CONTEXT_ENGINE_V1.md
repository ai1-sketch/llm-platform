...
- ✅ **M4b (التقاط المحادثة، ذاكرة عرضية)** — **مهمة خلفية في pre_call** (بلا كمون) تلتقط من **تاريخ الرسائل**: دورَ المستخدم الحالي + دورَ المساعد السابق في `conversation_memory` — يعمل **streaming وغير-streaming** (التاريخ مصدر موثوق مستقلّ عن التسليم؛ قياسٌ حيّ أثبت أن deltas/response لا تُعطي نصّ المساعد عند streaming، عكس التاريخ). scoped بـ `x-openwebui-chat-id`، dedup بفهرس فريد `(user_id, conversation_id, content_hash)`، importance=0.4 (< الحقائق)، 'تذكّر:' يُتخطّى. re-entrant-safe (نداء التضمين الداخلي بلا chat-id → لا التقاط). نقطة `/v1/conversation/capture`. **مُتحقَّق حيّاً:** حقيقة من دور **المستخدم** ومن دور **المساعد** (streaming) استُرجِعتا في محادثة **منفصلة** (طوكيو، مهندس طيران، تيسلا). 72 pytest + 5 تكامل. **مؤجَّل v2:** آخر دور مساعد في محادثة مهجورة (يُلتقَط عند الاستئناف)، تلخيص/تكثيف conversation_memory المتنامي.
