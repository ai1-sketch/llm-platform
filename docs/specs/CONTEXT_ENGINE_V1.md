...
## 8. التضمين والعربية
- **حاوية `embeddings` صغيرة** (Qwen3-Embedding-0.6B-GGUF، CPU، عقد `/v1/embeddings`) — لا تزاحم Gemma على الـ GPU. `memory` يطلب التضمين **عبر البوّابة** (موديل `embed-default`، [ADR-023](../DECISIONS.md): كل مرور موديل عبر البوّابة بلا استثناء — للتوسّع)؛ التبديل لمزوّد managed = سطر `api_base`.
...
- ✅ **M4b (التقاط المحادثة، ذاكرة عرضية)** — التقاط الدور (دور المستخدم + **إجابة الموديل**، `content` لا `reasoning`) **وقت توليده** عبر hooks ما-بعد-الرد: غير-streaming بـ `async_post_call_success_hook` (`response.choices[0].message.content`)، streaming بـ `async_post_call_streaming_iterator_hook` (**تمرير-أولاً صارم** للـ chunks ثم تجميع `delta.content`، فلا يتأثر المسار الحرج). يُخزَّن في `conversation_memory` عبر **مهمة خلفية** (بلا كمون) → نقطة `/v1/conversation/capture`. **لماذا ما-بعد-الرد لا pre_call:** التقاط إجابة الموديل وقت توليدها هو الحلّ الصحيح (لا اعتماد على تاريخ الرسائل المتأخّر دوراً). scoped بـ `x-openwebui-chat-id`، dedup بفهرس فريد `(user_id, conversation_id, content_hash)` (idempotent رغم تكرار الـ hooks)، importance=0.4 (< الحقائق)، 'تذكّر:' يُتخطّى (يُكتب عبر مسار WRITE الصريح). re-entrant-safe (نداء التضمين الداخلي بلا chat-id → لا التقاط). **مُتحقَّق حيّاً:** إجابة موديل **streaming** ("المشتري"، غائبة عن السؤال) وإجابة **غير-streaming** + حقيقة مستخدم استُرجِعت كلها في محادثة **منفصلة** عبر `/v1/assemble`؛ 4 صفوف فقط (لا تلوّث من نداءات التضمين). 74 pytest + 5 تكامل. **مؤجَّل v2:** تلخيص/تكثيف `conversation_memory` المتنامي، حواف آخر-دور في تدفّق مقطوع.
- **التالي: M5** — ذاكرة الملفّات (هضم + تقطيع) + eval harness (golden-set).
