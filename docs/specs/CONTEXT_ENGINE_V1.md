...
- ✅ **M4b (التقاط المحادثة، ذاكرة عرضية)** — جسر `pre_call→log_success` (post-response، بلا كمون): يلتقط **دور المستخدم** (streaming + غير-streaming) + **دور المساعد لغير-streaming** في `conversation_memory` (scoped بـ `x-openwebui-chat-id`، dedup بفهرس `content_hash`، importance=0.4 < الحقائق). re-entrant-safe (نداء التضمين الداخلي بلا stash → لا حلقة). نقطة `/v1/conversation/capture`. **مُتحقَّق حيّاً:** حقيقة ذُكِرت في محادثة (بلا "تذكّر:") استُرجِعت في محادثة **منفصلة** (عابر للجلسات، streaming كذلك). 71 pytest + 5 تكامل. **مؤجَّل v2:** دور المساعد عند streaming (يحتاج `streaming_iterator_hook`)، تلخيص/تكثيف conversation_memory المتنامي.
